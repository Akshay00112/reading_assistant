...
if not (shutil.which("espeak") or shutil.which("espeak-ng")):
    print("WARNING: eSpeak not available in PATH. Pronunciation features will be limited.")
...
def create_app():
    """
    Initialize the Flask application, database, and all routes.
    """
    app = Flask(__name__)
    ...
    # =========================
    # Each blueprint handles a specific feature of the app
    app.register_blueprint(pdf_bp, url_prefix='/api/pdf')          # PDF upload & reading
    app.register_blueprint(practice_bp, url_prefix='/api/practice') # Pronunciation practice
    app.register_blueprint(selection_bp, url_prefix='/api/selection') # Text selection
